...
- Secret values are keychain-owned and must never enter canonical content,
  sync payloads, diagnostics, activity, logs, or API responses.
...
The repository contains the strict TypeScript/Bun core, six provider adapters,
schema-v2 canonical library, local SQLite state, safe provider writes and
backups, project discovery and promotion, canonical sync protocol, Hono runtime,
headless CLI, Raycast-style manager client, and hardened Electron shell. These
surfaces are covered by unit and integration tests.
...
canonical content in plaintext unless its operator encrypts storage.
