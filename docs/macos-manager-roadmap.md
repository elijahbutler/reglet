...
- [ ] Define versioned JSON schemas for capability scan, source inventory, enrollment matrix, Master draft summary, effective composition, structured plan, drift item, and receipt detail.
...
- [ ] Represent saved Master revision and last-applied composition so "changes ready" is exact rather than inferred from timestamps.
