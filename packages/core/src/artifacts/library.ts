...
/**
 * Creates schema v2 metadata for a pre-v2 library without moving canonical
 * content. IDs are derived from the legacy locator so an interrupted migration
 * can be rerun before the manifest write without producing new identities.
 */
export async function migrateLibraryToV2(home = regletHome()): Promise<LibraryManifest> {
  const existing = await loadLibraryManifest(home);
  if (existing.artifacts.length > 0 || (await fileExists(libraryManifestPath(home)))) {
    return existing;
  }
  ...
  const artifacts = [
    ...
    ...(await discoverLegacySkills(home, targetsByKind.skill)),
    ...(await discoverLegacyMcpServers(home, targetsByKind.mcp)),
  ];
...
export async function addLibraryArtifact(
  ...
  if (
    manifest.artifacts.some(
      (candidate) => candidate.kind === artifact.kind && candidate.slug === artifact.slug,
    )
  ) {
...
export async function duplicateLibraryArtifact(
  ...
  const slug = nextAvailableSlug(
    `${source.slug}-copy`,
    manifest.artifacts.filter((artifact) => artifact.kind === source.kind),
  );
...
export async function readArtifactText(
  ...
  return `${JSON.stringify(servers[artifact.locator.serverName] ?? null, null, 2)}\n`;
}
...
export async function commitArtifactText(
  ...
  if (artifact.locator.type === 'file') {
    ...
    const servers = isRecord(value.mcpServers) ? { ...value.mcpServers } : {};
    servers[artifact.locator.serverName] = definition;
...
async function discoverLegacyInstructions(
  ...
  return files
    ...
    .map((file) => {
      const relPath = path.join('rules', file);
...
async function discoverLegacySkills(
  ...
  return entries
    ...
    .map((entry) => {
      const relPath = path.join('skills', entry.name);
...
function locatorForRename(locator: ArtifactLocator, slug: string): ArtifactLocator {
  if (locator.type === 'file') {
    return {
      type: 'file',
      path: path.join(path.dirname(locator.path), `${slug}${path.extname(locator.path)}`),
    };
  }
  if (locator.type === 'directory') {
    return {
      type: 'directory',
      path: path.join(path.dirname(locator.path), slug),
    };
  }
  return {
    ...locator,
...
function parseLibraryManifest(value: unknown): LibraryManifest {
